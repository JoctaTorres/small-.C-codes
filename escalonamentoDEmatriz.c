...
int main()
{
    ...
    float laux[co];
    float aux, k;
    for(ic= 0, il=0; il<li; ic++, ++il) //  loop principal para resolver a matriz
    {
        ...
        // definindo o primeiro elemento não nulo da linha como um
        aux=matriz[il][ic];
        for(oc=ic; oc<co; oc++)
        {
            matriz[il][oc]/=aux;
        ...
        {
            k=-matriz[lo][ic];
            for(oc=ic; oc<co; oc++) // usando uma linha auxiliar: copia e opera a linha selecionada
            {
                laux[oc] = matriz[il][oc];
                laux[oc]*=k;
